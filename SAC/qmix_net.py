...
class QMixNet(nn.Module):
    ...
    def forward(self, q_values, states):  # states的shape为(episode_num, max_episode_len， state_dim)
        ...
        states = states.reshape(-1, self.input_dim)  # (episode_num * max_episode_len, state_dim) input_dim=state_dim

        w1 = torch.abs(self.hyper_w1(states))  # (1920,160) ## (-1,state_dim) --> (-1,n_agents*qmix_hidden_dim)
        b1 = self.hyper_b1(states)  # (1920, 32) ## (-1, qmix_hidden_dim)
        ...
        q_total = q_total.view(episode_num, -1, 1)  # (32, 60, 1) ## (episode_num, max_episode_len, 1)
        return q_total
